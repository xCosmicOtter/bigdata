...
EPITA's computers using podman, **some docker functionnalities may crash during the first start**. It usually happen after the docker images and boursorama.tar download (when the analyzer's job begins **~8min after the ```make all``` command**). If so, just ```Ctrl-C``` and run the ```make``` command again.
